"""The shared look and vocabulary for every talk: a few-colour style in the spirit of 3Blue1Brown, built with Manim
Community Edition. Import it in every scene file with `from lib.palette import *`.
...
Rules the helpers encode (see docs/principles.md for the why):
  ...
  * nothing here hard-codes a colour, a font, a corner or a stroke: they come from the theme, so a deck written once
    presents in any of the styles AGENTS.md lists
...
TEXT = THEME["text"]               # body text, titles
CAPTION = THEME["caption"]         # the footnote at the bottom of a step
...
def rad(k: float = 1.0) -> float:
    """A corner radius derived from the theme's, so every rounded thing squares off together."""
    return RADIUS * k
...
def box(w: float, h: float, name: str = "", color: str = A3, size: float = 22, fill: float = None, name_align: str = "center") -> VGroup:
    ...
    r = RoundedRectangle(corner_radius=rad(), width=w, height=h, stroke_color=color, stroke_width=sw(), fill_color=color,
                         fill_opacity=FILL if fill is None else fill)
    g = VGroup(r)
    if name:
...
def node(name: str, color: str = A1, w: float = 2.2, h: float = 0.8, size: float = 20, sub: str = "") -> VGroup:
    ...
    r = RoundedRectangle(corner_radius=rad(0.8), width=w, height=h, stroke_color=color, stroke_width=sw(0.88), fill_color=color, fill_opacity=FILL * 1.2)
...
def arrow(a: Mobject, b: Mobject, text: str = "", color: str = MUTED, buff: float = 0.12, size: float = 14, above: bool = True) -> VGroup:
    ...
    ar = Arrow(start, end, buff=0, color=color, stroke_width=sw(0.88), tip_length=0.18)
    g = VGroup(ar)
    if text:
...
def travel(scene, path_from: Mobject, path_to: Mobject, color: str = A1, radius: float = 0.09, run_time: float = 0.5, flash: str = "",
...
def dashed(a: Mobject, b: Mobject, text: str = "", color: str = MUTED) -> VGroup:
    ...
    ln = DashedLine(edge_point(a, b.get_center()), edge_point(b, a.get_center()), color=color, stroke_width=sw(0.64), dash_length=0.1, stroke_opacity=0.7)
    g = VGroup(ln)
    if text:
...
def column(n: int = 8, color: str = A1, cell: float = 0.08, op: float = None) -> VGroup:
    """A vector: a thin column of cells."""
    return VGroup(*[Square(cell, fill_color=color, fill_opacity=SOLID if op is None else op, stroke_width=0) for _ in range(n)]).arrange(DOWN, buff=0.012)


def grid(rows: int, cols: int, color: str = A3, cell: float = 0.08, op: float = None) -> VGroup:
    """A matrix or a memory: a grid of cells, indexed row * cols + col."""
    return VGroup(*[Square(cell, fill_color=color, fill_opacity=SOLID * 0.67 if op is None else op, stroke_width=0) for _ in range(rows * cols)]).arrange_in_grid(rows=rows, cols=cols, buff=0.012)
...
class Gauge(VGroup):
    """A vertical gauge with a name; set(level) returns an animation filling it 0..1. The alert accent above 0.9. Two gauges
    ...
    def __init__(self, name: str, color: str, height: float = 1.6, **kw):
        super().__init__(**kw)
        self.frame = RoundedRectangle(corner_radius=rad(0.4), width=0.36, height=height, stroke_color=DIM, stroke_width=sw(0.8), fill_opacity=0)
        self.fill = Rectangle(width=0.30, height=0.01, fill_color=color, fill_opacity=SOLID, stroke_width=0).move_to(self.frame.get_bottom() + UP * 0.03, aligned_edge=DOWN)
    ...
    def set(self, level: float):
        ...
        target = Rectangle(width=0.30, height=self.h * level, fill_color=ALERT if level > 0.9 else self.color, fill_opacity=SOLID, stroke_width=0)
...
class Bars(VGroup):
    ...
    def __init__(self, heights, width: float = 0.16, gap: float = 0.05, color: str = A2, emphasise_max: bool = True, **kw):
        super().__init__(**kw)
        top = max(heights)
        for h in heights:
            op = (SOLID if h == top else SOLID * 0.4) if emphasise_max else SOLID
...
def timeline(y: float, x0: float, segments, colors: dict, height: float = 0.3, scale: float = 1.0) -> VGroup:
    ...
    for kind, length in segments:
        w = length * scale
        g.add(Rectangle(width=w - 0.03, height=height, fill_color=colors[kind], fill_opacity=SOLID * 0.95, stroke_width=0).move_to([x + w / 2, y, 0]))
        x += w
    return g
...
def vector(seed: int, color: str = A1, n: int = 8, cell: float = 0.16) -> VGroup:
    """A vector whose cells carry different shades, so two vectors are told apart. Same seed, same vector."""
    import random
    rnd = random.Random(seed)
    return VGroup(*[Square(cell, fill_color=color, fill_opacity=SOLID * (0.33 + 0.67 * rnd.random()), stroke_width=0) for _ in range(n)]).arrange(DOWN, buff=cell * 0.15)
...
def sweep(scene, vec, vcolor, jobs, cols: int = 8, rt: float = 0.14, hi: str = HI, mat_color: str = A3):
...
class Log(VGroup):
    ...
    def __init__(self, x0: float, y: float, capacity: int = 12, base: int = 0, name: str = "", cell: float = 0.42, gap: float = 0.08, **kw):
        ...
        self.rail = Rectangle(width=capacity * self.pitch + gap, height=cell + 0.16, stroke_color=DIM, stroke_width=sw(0.48), fill_opacity=0)
    ...
    def _cell(self, i: int, color: str) -> Square:
        return Square(self.side, fill_color=color, fill_opacity=SOLID, stroke_width=0).move_to(self.slot(i))
...
class Pointer(VGroup):
    ...
    def __init__(self, name: str, color: str = A4, **kw):
        super().__init__(**kw)
...
def block(text: str, color: str, w: float = 3.4, h: float = 0.35, size: float = 14, bare: bool = False) -> VGroup:
    ...
    r = RoundedRectangle(corner_radius=rad(0.4), width=w, height=h, fill_color=color, fill_opacity=FILL * 1.6, stroke_color=color, stroke_width=sw(0.56))
    bar = Rectangle(width=0.09, height=h, fill_color=color, fill_opacity=SOLID, stroke_width=0).move_to(r.get_left(), aligned_edge=LEFT)
    if bare:
        return VGroup(r, bar)
...
def code(source, language: str = "python", size: float = 18, width: float = 0.0) -> VGroup:
    """Syntax-highlighted code as a block: a panel with the lines highlighted by Pygments (the theme names the style;
    it must keep plain identifiers neutral, as monokai and xcode do and one-dark does not), no
    ...
    c = Code(code_string=text, language=language, formatter_style=CODE_STYLE, add_line_numbers=False, background="rectangle",
             background_config={"fill_color": PANEL, "fill_opacity": 1.0, "stroke_color": DIM, "stroke_width": sw(0.48), "corner_radius": rad(0.8), "buff": 0.45},
