"""Move 1: kubectl apply writes a record, not a process.
  ...
  Final frame: kubectl top-left, the API server bar with its three gates, etcd bottom-left holding one Deployment card,
  three empty nodes bottom-right, two counters between them: 3 desired, 0 running.
...
class ApplyRequest(TalkSlide):
    def construct(self):
        ...
        S = Stage()
        manifest = code(MANIFEST, "yaml", 17).move_to([X_NODES_L, Y_API - H_API / 2 - GAP, 0], aligned_edge=UL)   # what the user typed, where the nodes will appear
        ml = label("what kubectl apply sends (trimmed)", 15, MUTED).next_to(manifest, DOWN, buff=GAP_TIGHT).align_to(manifest, LEFT)
        ...
        server, the front door of the cluster, drawn as a bar because everything in this talk passes through it; under it, at
        the left, etcd, the database that holds every record. Right, the manifest the user is about to apply, trimmed to the
        lines that matter: a Deployment, three replicas of one image. Nothing has been sent yet.""")
        req = card("Deployment", "3 replicas, v1").next_to(S.client, RIGHT, buff=GAP_WIDE)
        ...
        self.play(frame.animate.scale(ZOOM).move_to([X_GATES[1] + 0.15, Y_API, 0]), FadeOut(t), FadeOut(S.store), FadeOut(S.client), FadeOut(manifest), FadeOut(ml),
                  req.animate.scale(0.6).move_to(gates[0].get_center() + LEFT * 0.55), run_time=1.0)   # from kubectl into the bar, before the first gate
        for k in range(3):
            if k:
        ...
        self.play(req.animate.move_to(gates[2].get_center() + RIGHT * 0.55), run_time=0.4)   # past the last gate; the next motion is the write into etcd's slot
        ...
        # --- nothing runs yet
        bar = highlight_line(manifest, 2)                                       # replicas: 3 is the number the gap is measured against
