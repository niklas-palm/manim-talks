...
class Quantisation(TalkSlide):
    def construct(self):
        ...
        cache, not the weights, is what limits how many requests fit. The bottom number is prefill work, prompt tokens per
        second, the measured throughput of one GPU inside an eight-second latency budget.""")
        ...
        measure on any task; 4-bit costs a little, always. Hold the question until then.""")
...
class MixtureOfExperts(TalkSlide):
    def construct(self):
        ...
        random.seed(3)
        for _ in range(3):
            tok = Square(0.3, fill_color=OUTPUT, fill_opacity=0.9, stroke_width=0).next_to(layers, UP, buff=0.08)
        ...
        concurrency and shrinks as the batch grows. Size a fleet on the loaded number, never the batch-of-one number. Two knobs so far, both about bytes per step. The third is about not reading at all.""")
