...
class GPU(VGroup):
    ...
    def __init__(self, name: str = "one GPU", w: float = 5.2, mem_gb: float = 96, weights_gb: float = 57, **kw):
        super().__init__(**kw)
        self.outer = RoundedRectangle(corner_radius=rad(1.2), width=w, height=3.6, stroke_color=WEIGHTS, stroke_width=sw(), fill_color=WEIGHTS, fill_opacity=FILL * 0.6)
        self.title = label(name, 22, WEIGHTS).next_to(self.outer.get_top(), DOWN, buff=0.15)
        bar_w = w - 1.9
        self.mem_gb, self.bar_w = mem_gb, bar_w
        self.mem = Rectangle(width=bar_w, height=0.55, stroke_color=DIM, stroke_width=sw(0.8), fill_opacity=0).move_to(self.outer.get_center() + UP * 0.55 + LEFT * 0.7)
        self.weights = Rectangle(width=bar_w * weights_gb / mem_gb, height=0.51, fill_color=WEIGHTS, fill_opacity=SOLID * 0.94, stroke_width=0).align_to(self.mem, LEFT).set_y(self.mem.get_y())
        self.cache = Rectangle(width=0.01, height=0.51, fill_color=CACHE, fill_opacity=SOLID * 0.94, stroke_width=0).next_to(self.weights, RIGHT, buff=0).set_y(self.mem.get_y())
        ...
        self.cores = VGroup(*[Square(side, fill_color=DIM, fill_opacity=SOLID, stroke_width=0) for _ in range(32)]).arrange_in_grid(rows=2, cols=16, buff=0.05)
    ...
    def set_cache(self, gb: float, color: str = CACHE):
        ...
        target = Rectangle(width=w, height=0.51, fill_color=color, fill_opacity=SOLID * 0.94, stroke_width=0).next_to(self.weights, RIGHT, buff=0).set_y(self.mem.get_y())
...
def gauge_at(g: Gauge, level: float):
    ...
    target = Rectangle(width=0.30, height=g.h * level, fill_color=HOT if level > 0.9 else g.color, fill_opacity=SOLID, stroke_width=0)
...
def cache_at(gpu: GPU, gb: float, color: str = CACHE):
    ...
    gpu.cache.become(Rectangle(width=w, height=0.51, fill_color=color, fill_opacity=SOLID * 0.94, stroke_width=0).next_to(gpu.weights, RIGHT, buff=0).set_y(gpu.mem.get_y()))
    return gpu
