...
class Rounding(TalkSlide):
    def construct(self):
        t = title(self, "What rounding a weight does to an answer", "5  what precision costs in answers")
        ...
        cap = swap_caption(self, cap, "Every weight moves a little, up or down; every score the model computes moves a little")
        self.next_slide("""Now every weight in the matrix has done the same, each by its own small amount, some up, some down. A layer's
        ...
        cap = swap_caption(self, cap, "Where the model was sure, nothing changes. Where it was nearly undecided, the choice flips.")
        ...
        self.play(FadeOut(val), FadeIn(b1), FadeIn(b2), FadeIn(l1), FadeIn(l2), FadeIn(pick(b1, OUTPUT)), FadeIn(pick(b2, OUTPUT)), run_time=0.7)
...
class PrecisionCost(TalkSlide):
    def construct(self):
        ...
        reset(self, grid, idx)
        cap = swap_caption(self, cap, "Calibrated 4-bit: 6 to 10 of 100 change, 4 to wrong for every 3 to right")
        idx = flip(self, grid, 3, 4, seed=3)
        self.next_slide("""Four-bit weights are the first precision with a consistent cost, and it is the coarse levels from the rounding
