...
class TheCode(TalkSlide):
    def construct(self):
        ...
        msgs = Messages(-4.9, 1.2, S)
        sysb = block("system", "", S, bare=True).move_to([-4.9, 1.45, 0])
        ...
        hl = Rectangle(width=code.width + 0.3, height=0.4, fill_color=TOOL, fill_opacity=0.14, stroke_width=0)
        ...
        self.play(at(2), run_time=0.25); msgs.append(self, block("tool_use", "", S, bare=True), frm=model[0], run_time=0.35)
        self.play(at(3), run_time=0.3)
        self.play(at(4), run_time=0.25); travel(self, card, dev, TOOL, run_time=0.3, flash=RESULT)
        self.play(at(5), run_time=0.25); msgs.append(self, block("tool_result", "", S, bare=True), frm=card, run_time=0.35)
        self.play(at(6), run_time=0.3); self.play(at(0), run_time=0.3)
        self.play(at(1), run_time=0.25); call_model(self, msgs, model, run_time=0.45, extra=[sysb, card])
        self.play(at(2), run_time=0.25); a = msgs.append(self, block("assistant", "", S, bare=True), frm=model[0], run_time=0.35)
