...
TITLES = {"api": ("An application with one model call", "1  a plain application"),
          ...
          "agent": ("The agent, as one thing", "7  one thing inside the application")}
...
HIST_CODE = [("user", "user · add a retry to the upload function"), ("tool_use", 'assistant · tool_use: bash("grep -rn upload src/")'),
             ...
             ("tool_result", "user · tool_result: written"), ("tool_use", 'assistant · tool_use: bash("pytest tests/test_upload.py")'),
             ("tool_result", "user · tool_result: 3 passed"), ("assistant", "assistant · added a retry with backoff; tests pass")]
HIST_CODE_KINDS = [kind for kind, _ in HIST_CODE]
BASE_TOOLS = [g[0] for g in GENERIC_TOOLS]


def mini_block(kind: str, w: float = ENTITY["row_w"], rows: int = 1) -> VGroup:
    """A message as the entity draws it: a thin bare line; `rows` > 1 draws a taller one for a message that is visibly bigger."""
    return block("", KIND[kind], w=w, h=ROW_H * rows + ROW_GAP * (rows - 1), bare=True)


def compact_card(name: str, w: float) -> VGroup:
    """A tool as the entity draws it: a card carrying the tool's name, larger than the lines of context beside it."""
    r = Rectangle(width=w, height=0.3, fill_color=TOOL, fill_opacity=FILL * 1.2, stroke_color=TOOL, stroke_width=sw(0.48))
    return VGroup(r, label(name, 12, TEXT).move_to(r))


def agent_entity(kinds, names=tuple(BASE_TOOLS), cx: float = ENTITY["cx"], cy: float = ENTITY["cy"], w: float = ENTITY["w"], h: float = ENTITY["h"],
                 gap: float = ENTITY["gap"], row_w: float = ENTITY["row_w"], card_w: float = ENTITY["card_w"]) -> tuple:
    """The agent as one thing: (frame, list, cards). kinds: the rows' message kinds; names: the cards. The list fills the
    left of the frame, the cards the right; the list remembers its row width so a scene can add rows that fit."""
    frame = box(w, h, "agent", MUTED, size=12, fill=FILL * 0.6, name_align="left").move_to([cx, cy, 0])
    left, right, top = cx - w / 2 + 0.15, cx + w / 2 - 0.15, cy + h / 2
    lst = Stack(left + row_w / 2, top - 0.32, h=ROW_H, gap=ROW_GAP)
    lst.row_w = row_w
    for kind in kinds:
        b = mini_block(kind, row_w).move_to(lst.slot(len(lst.blocks))); lst.blocks.append(b); lst.add(b)
    cards = VGroup(*[compact_card(n, card_w).move_to([right - card_w / 2, top - 0.42 - gap * i, 0]) for i, n in enumerate(names)])
    return frame, lst, cards


class Run:
    """The loop at speed through the entity: rows join its list, the list goes to the model, a tool call travels to its
    card and on to what the tool reaches, results come back, the counters count. `on_model` and `on_tool` let a scene
    draw something more each time."""

    def __init__(self, scene, lst: Stack, cards: VGroup, model: VGroup, calls: Counter, tools: Counter, n_calls: int = 0, n_tools: int = 0):
        self.s, self.lst, self.cards, self.model, self.calls, self.tools = scene, lst, cards, model, calls, tools
        self.n_calls, self.n_tools = n_calls, n_tools
        self.on_model = self.on_tool = None
    ...
    def think(self, rt: float = 0.25):
        call_model(self.s, self.lst, self.model, extra=self.cards, run_time=rt)
        self.n_calls += 1
        self.s.play(self.calls.to(self.n_calls), run_time=rt / 2)
        if self.on_model:
            self.on_model()

    def use(self, card: int, target=None, rt: float = 0.25):
        b = self.lst.append(self.s, self.row("tool_use"), frm=self.model[0], run_time=rt)
        travel(self.s, b, self.cards[card], TOOL, edges=True, run_time=rt)
        if target is not None:
            travel(self.s, self.cards[card], target, TOOL, edges=True, run_time=rt)
        self.n_tools += 1
        self.s.play(self.tools.to(self.n_tools), run_time=rt / 2)
        if self.on_tool:
            self.on_tool()
        return b

    def result(self, frm, rt: float = 0.25, rows: int = 1):
        return self.lst.append(self.s, self.row("tool_result", rows), frm=frm, run_time=rt)

    def answer(self, rt: float = 0.25):
        return self.lst.append(self.s, self.row("assistant"), frm=self.model[0], run_time=rt)
