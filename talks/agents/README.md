...
A 17-minute talk for engineers who have used chat assistants and want to see what an agent is, in the machinery. A plain
...
| 7. The agent, as one thing | `TheAgent` | 2 | 1.5 |
...
loop scene draws that properly on its first call. The agent is drawn as one compact entity (a frame holding the messages as bare rows and the tools as named cards)
only in the last move, after every part has been seen at full size; an earlier attempt to introduce the frame in the middle
of the framework move split the picture without adding anything. Tool cards carry three lines (name
