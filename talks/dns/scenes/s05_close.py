...
class Standing(TalkSlide):
    def construct(self):
        ...
        # --- anycast
        names = VGroup(*[label(f"{c}.root-servers.net.", 12, ZONE) for c in "abcdefghijklm"]).arrange(DOWN, aligned_edge=LEFT, buff=0.04).move_to([-5.4, 0.2, 0], aligned_edge=LEFT)
        nl = label("13 server names; 12 operators", 14, MUTED).next_to(names, UP, buff=0.15).align_to(names, LEFT)
        self.play(FadeIn(names, lag_ratio=0.05), FadeIn(nl), run_time=0.8)
        import random
        random.seed(3)
        world = Ellipse(width=7.5, height=4.2, color=DIM, stroke_width=1.5).move_to([1.6, 0.1, 0])
        sites = VGroup(*[Dot(radius=0.045, color=ZONE).move_to([1.6 + random.uniform(-3.4, 3.4), 0.1 + random.uniform(-1.8, 1.8), 0]) for _ in range(180)])
        sites = VGroup(*[d for d in sites if ((d.get_x() - 1.6) / 3.75) ** 2 + ((d.get_y() - 0.1) / 2.1) ** 2 < 1])
        sl = label("2,045 anycast instances of those 13 names, one dot per ~12", 13, MUTED).next_to(world, DOWN, buff=0.1)
        self.play(FadeIn(world), FadeIn(sites, lag_ratio=0.005), FadeIn(sl), run_time=1.4)
        you = Dot(radius=0.09, color=NAME).move_to([3.4, -0.9, 0])
        yl = label("a resolver in Sydney asks a.root-servers.net", 12, NAME).next_to(you, DOWN, buff=0.06)
        ...
        self.play(Create(path), nearest.animate.set_color(ADDRESS).scale(2.5), run_time=0.6)
        al = label("one address, announced from many places: routing delivers the packet to the nearest", 13, MUTED).move_to([0.6, -2.7, 0])
        ...
        pkt = VGroup(Rectangle(width=6.0, height=0.6, stroke_color=NAME, stroke_width=2, fill_color=NAME, fill_opacity=0.08),
                     label("header: id, flags (RD, RA, TC), counts", 13, TEXT), label("question: name, type, class", 13, NAME)).arrange(DOWN, buff=0.06)
        pkt[1].move_to(pkt[0].get_center() + UP * 0.12); pkt[2].move_to(pkt[0].get_center() + DOWN * 0.12)
        pkt.move_to([0, 1.2, 0])
        pl = label("a question is one UDP packet to port 53", 14, MUTED).next_to(pkt, UP, buff=0.12)
        self.play(FadeIn(pkt), FadeIn(pl), run_time=0.6)
        ans = VGroup(Rectangle(width=6.0, height=0.9, stroke_color=ADDRESS, stroke_width=2, fill_color=ADDRESS, fill_opacity=0.08),
                     label("answer records", 13, ADDRESS), label("authority records (the NS pointers)", 13, ZONE), label("additional records (glue: the servers' addresses)", 13, ZONE)).arrange(DOWN, buff=0.04)
        for i, m in enumerate(ans[1:]):
            m.move_to(ans[0].get_center() + UP * (0.25 - 0.25 * i))
        ans.move_to([0, -0.4, 0])
        limit = label("512 bytes fit in the classic UDP answer; a longer one sets the TC flag and the client asks again over TCP", 13, MUTED).next_to(ans, DOWN, buff=0.15)
        ...
        root = zone_box(".  the root zone", "pointers to 1,393 top-level domains, kept for 6 days", Y_ROOT, w=5.4)
        tld = zone_box("com.", "pointers to its names' servers, kept for 2 days", Y_TLD, w=5.4)
        auth = zone_box("example.com.", "the addresses, kept for 5 minutes", Y_AUTH, w=5.4)
        ...
        rules = VGroup(label("every answer is the address, or a pointer to who to ask next", 18, TEXT),
                       label("every level remembers what it heard for exactly as long as it was told to", 18, TEXT)).arrange(DOWN, aligned_edge=LEFT, buff=0.1).move_to([0, -2.75, 0])
        rules[0][15:22].set_color(ADDRESS); rules[0][28:35].set_color(ZONE); rules[1][12:21].set_color(REMEMBERED)
