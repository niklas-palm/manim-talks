...
two ways as it grows: the list is too big to give everyone, and a change has to reach every copy.
  ...
  Final frame (move two starts from it): the laptop on the left; a list of large name -> address rows in the centre that has run off the bottom
  of the frame; a counter of names; three copies of the list at the right, two of them stale after an edit.
  Clicks: 1 the still: one laptop, one short list  2 a lookup succeeds  2 the list grows past the frame, the counter says how far
...
class OneList(TalkSlide):
    def construct(self):
        ...
        fade_below(more)   # the list continues below the frame: fade it towards the edge instead of cutting it
        ...
        new = label(NEW_ADDRESS, 16, HOT).move_to(lst[0][3], aligned_edge=LEFT)
        self.play(Transform(lst[0][3], new), lst[0][0].animate.set_fill(HOT, 0.25), run_time=0.6)
        c = copies[0]
        self.play(c[0][0].animate.set_fill(HOT, 0.25), Transform(c[0][3], label(NEW_ADDRESS, 15, HOT).move_to(c[0][3], aligned_edge=LEFT)), run_time=0.4)
