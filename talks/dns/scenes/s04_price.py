...
RES_X = [-5.4, -3.0, -0.6, 1.8, 4.2]


def resolver_small(x: float, y: float, name: str) -> VGroup:
    g = box(2.1, 1.05, name, ZONE, size=13)
    g.move_to([x, y, 0])
    return g
...
class Price(TalkSlide):
    def construct(self):
        ...
        auth = zone_box("example.com.", "the owner's zone", 2.0, w=6.0, h=1.0, x=0.6)
        r_auth = record("www.example.com.", "A", "104.20.23.154", "5 min", ADDRESS).move_to([0.6, 1.7, 0])
        self.play(FadeIn(auth), FadeIn(r_auth), run_time=0.5)
        rl = label("resolvers around the world, each with its own copy", 14, MUTED).move_to([-0.6, 0.55, 0])
        rs = VGroup(*[resolver_small(x, -0.2, n) for x, n in zip(RES_X, ("Stockholm", "Dublin", "Virginia", "Tokyo", "Sydney"))])
        copies = VGroup(*[record("www", "A", "104.20.23.154", "", ADDRESS, width=1.9).scale(0.85).move_to(r[0].get_center() + DOWN * 0.15) for r in rs])
        fuses = VGroup(*[fuse(c, f) for c, f in zip(copies, (0.9, 0.3, 0.6, 0.15, 0.75))])
        self.play(FadeIn(rl), FadeIn(rs), FadeIn(copies), FadeIn(fuses), run_time=0.7)
        # --- the change
        new_val = label("104.20.24.1", 13, HOT).move_to(r_auth[3], aligned_edge=LEFT)
        self.play(Transform(r_auth[3], new_val), r_auth[0].animate.set_fill(HOT, 0.25), run_time=0.6)
        stale = label("stale copies: the old address, until each expires", 13, HOT).move_to([-0.6, -1.15, 0])
        self.play(FadeIn(stale), *[c[0].animate.set_fill(HOT, 0.2) for c in copies], run_time=0.5)
        clock = label("time", 13, MUTED).move_to([-6.4, -1.75, 0])
        self.play(FadeIn(clock), run_time=0.2)
        order = sorted(range(5), key=lambda i: (0.9, 0.3, 0.6, 0.15, 0.75)[i])
        for i in order:
            fresh = label("104.20.24.1", 13, ADDRESS).scale(0.85).move_to(copies[i][3], aligned_edge=LEFT)
        ...
        they expire one by one: Tokyo first, then Dublin, Virginia, Sydney, Stockholm. Until then, users behind a stale
        ...
        trick = VGroup(label("1  set the TTL to 60 s and wait one old TTL", 14, TEXT), label("2  change the address: every copy expires within 60 s", 14, TEXT),
                       label("3  raise the TTL again", 14, TEXT)).arrange(DOWN, aligned_edge=LEFT, buff=0.08).move_to([-3.2, -1.5, 0], aligned_edge=LEFT)
        ttl_lab = label("60 s", 12, REMEMBERED).move_to(r_auth[4], aligned_edge=RIGHT)
        ...
        newer = label("104.20.25.9", 13, ADDRESS).move_to(r_auth[3], aligned_edge=LEFT)
        ...
        self.play(*[Transform(c[3], label("104.20.25.9", 13, ADDRESS).scale(0.85).move_to(c[3], aligned_edge=LEFT)) for c in copies],
                  *[Transform(f, fuse(c, 1.0)) for f, c in zip(fuses, copies)], run_time=0.5)
        self.play(FadeIn(trick[2]), Transform(r_auth[4], label("5 min", 12, REMEMBERED).move_to(r_auth[4], aligned_edge=RIGHT)), run_time=0.4)
        ...
        client = client_box(-0.3).shift(RIGHT * 0.6)
        res = box(3.2, 2.2, "recursive resolver", ZONE, size=15).move_to([-1.0, -0.5, 0])
        ...
        soa = record("example.com.", "SOA", "minimum 1800 s", "30 min", ZONE).move_to([0.6, 1.45, 0])
        self.play(r_auth.animate.shift(UP * 0.25), FadeIn(soa), run_time=0.4)
        answer(self, auth, res, HOT, "no such name (NXDOMAIN)")
        neg = record("wwww.example.com.", "does not exist", "", "30 min", HOT, width=3.0).scale(0.85).move_to(res[0].get_center() + DOWN * 0.2)
        self.play(FadeIn(neg), FadeIn(fuse(neg)), run_time=0.4)
        answer(self, res, client, HOT, "no such name")
        nl = label("the 'no' is cached too, for the zone's SOA minimum: 30 minutes here; RFC 2308 suggests one to three hours", 13, MUTED).move_to([0, -2.6, 0])
        ...
        # --- an alias
        self.play(FadeOut(neg), FadeOut(nl), run_time=0.3)
        cname = record("www.example.com.", "CNAME", "edge.cdn.example.net.", "1 h", ZONE).move_to([0.6, 1.7, 0])
        ...
        far = zone_box("cdn.example.net.", "another zone, another walk: root, net, example.net", -2.3, w=6.0, h=0.7, x=3.0)
        ...
        al = label("an alias is a pointer to another name; the resolver restarts the walk there (RFC 1034)", 13, MUTED).move_to([0, -2.9, 0])
