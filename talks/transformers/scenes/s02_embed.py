...
class Embedding(TalkSlide):
    def construct(self):
        ...
        sentence = label("The cat sat on the mat", 34, TOKEN).move_to([0, 2.45, 0])
        ...
        # --- tokens with ids
        toks = VGroup(*[VGroup(Square(0.72, fill_color=TOKEN, fill_opacity=0.85, stroke_width=0), label(w, 19, "#0f1116")) for w in WORDS])
        for g, x in zip(toks, XS):
            g.move_to([x, 1.65, 0]); g[1].move_to(g[0])
        ids = VGroup(*[label(i, 17, MUTED).next_to(g, DOWN, buff=0.12) for i, g in zip(IDS, toks)])
        ...
        bpe = label("byte-pair encoding, vocabulary of 50,257", 17, MUTED).to_edge(DOWN, buff=0.6)
        ...
        table = VGroup(*[Rectangle(width=1.3, height=0.16, stroke_color=DIM, stroke_width=1, fill_color=VIOLET, fill_opacity=0.25) for _ in range(12)]).arrange(DOWN, buff=0.035)
        table.move_to([-6.3, -0.4, 0])
        tl = label("embedding table\n50,257 rows", 15, WEIGHTS).next_to(table, UP, buff=0.15)
        self.play(FadeIn(table), FadeIn(tl))
        vecs = VGroup()
        rows = [2, 9, 6, 11, 4, 8]
        for k, (g, r) in enumerate(zip(toks, rows)):
            v = vector(20 + k, TOKEN, cell=0.16).next_to(g, DOWN, buff=0.55)
            vecs.add(v)
        ...
        vl = label("one vector per token: 512 numbers, drawn 8", 17, TOKEN).to_edge(DOWN, buff=0.6)
        self.play(FadeIn(vl))
        ...
        same = label('both "the" tokens: same id, same vector', 17, MUTED).to_edge(DOWN, buff=1.0)
        order = label("no order: attention sees a set", 17, RED).to_edge(DOWN, buff=0.6)
        ...
        pos = VGroup(*[vector(60 + k, ATTN, cell=0.16).next_to(v, DOWN, buff=0.3) for k, v in enumerate(vecs)])
        pl = label("position vector added, one per slot", 17, ATTN).to_edge(DOWN, buff=0.6)
