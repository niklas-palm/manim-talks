...
class Predict(TalkSlide):
    def construct(self):
        ...
        sentence = label("The cat sat on the", 34, TOKEN).move_to([-1.5, ROWS[0], 0])
        self.play(FadeIn(sentence))
        Y = -0.5
        vec = vector(30, TOKEN, cell=0.24).move_to([COLS[2], Y, 0])   # alone at the centre first; it slides left when the matrix appears
        ...
        vocab = grid(12, 8, WEIGHTS, cell=0.24).move_to([COLS[1], Y, 0])
        vocl = label("vocabulary matrix\n50,257 rows, one per token", 17, WEIGHTS).next_to(vocab, UP, buff=GAP)
        scores = column(12, TEXT, 0.24, op=0.0).move_to([-1.3, Y, 0])
        ...
        self.play(FadeOut(VGroup(vec, vll, scores, scl)), picked.animate.scale(0.5).move_to(newword.get_center()), run_time=0.7)
        ...
        tvi = VGroup(label("training: all positions at once", 22, MUTED), label("inference: one new token at a time", 22, MUTED)).arrange(DOWN, aligned_edge=LEFT, buff=GAP).move_to([COLS[0], ROWS[2] - 0.3, 0], aligned_edge=LEFT)
