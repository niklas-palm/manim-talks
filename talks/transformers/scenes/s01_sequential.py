...
class Sequential(TalkSlide):
    def construct(self):
        ...
        toks = VGroup(*[VGroup(Square(0.95, fill_color=TOKEN, fill_opacity=0.85, stroke_width=0), label(w, 24, "#0f1116")) for w in WORDS])
        toks.arrange(RIGHT, buff=0.75).move_to([0, 1.5, 0])
        for g in toks:
            g[1].move_to(g[0])
        self.play(LaggedStart(*[FadeIn(g, shift=DOWN * 0.15) for g in toks], lag_ratio=0.1), run_time=1.0)
        steps = Counter("sequential steps to read the sentence", 0, "", RED, size=26).move_to([-6.4, -2.4, 0], aligned_edge=LEFT)
        ...
        h = VGroup(RoundedRectangle(corner_radius=0.12, width=1.3, height=1.3, stroke_color=ATTN, stroke_width=3, fill_color=ATTN, fill_opacity=0.15),
                   label("state", 20, ATTN))
        h[1].move_to(h[0])
        h.next_to(toks[0], DOWN, buff=1.0)
        self.play(FadeIn(h))
        for k in range(N):
            self.play(h.animate.next_to(toks[k], DOWN, buff=1.0), run_time=0.35)
        ...
        steps2 = Counter("sequential steps to read the sentence", 1, "", GREEN, size=26).move_to([-6.4, -2.4, 0], aligned_edge=LEFT)
        conns = Counter("connections: every token to every other", 0, "", ATTN, size=26).move_to([0.4, -2.4, 0], aligned_edge=LEFT)
        ...
        for a in range(N):
            for b in range(N):
                if a != b:
                    ...
                    edges.add(ArcBetweenPoints(pa, pb, angle=PI / 2.2 if a < b else -PI / 2.2, color=ATTN, stroke_width=1.6, stroke_opacity=0.55))   # bows downward, below the tokens
