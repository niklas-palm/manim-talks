...
from six to one; a counter of connections shows the price. This is the deck's first frame: the title is written in
and the speaker introduces the talk over the six tokens.
"""
...
class Sequential(TalkSlide):
    def construct(self):
        ...
        times and the last position predicts the next word. Five moves, on one picture that grows from these six tokens:
        the parallel idea, words becoming vectors, attention, the token thinking alone many times, and one token at a time.
        A sentence, six tokens. The question is how a network turns these into an understanding of the sentence. For years
        ...
        steps2, conns = sequential_counters(1, 0)
        self.play(FadeIn(steps2), FadeIn(conns))
        edges = all_edges(toks)
