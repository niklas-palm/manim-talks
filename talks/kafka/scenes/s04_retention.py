...
class Retention(TalkSlide):
    def construct(self):
        ...
        segs = VGroup(*[broker(n, 3.95, 1.2).move_to([x, SY, 0]) for n, x in zip(("00000000000000000000.log", "00000000000000000009.log", "00000000000000000018.log"), SEGX)])
        logs = [Log(x - 1.8, SY - 0.12, capacity=9, base=b) for x, b in zip(SEGX, (0, 9, 18))]
        for i, l in enumerate(logs):
            for k in range(9 if i < 2 else 5):
                l.put(SEQ[(i * 9 + k) % len(SEQ)])
        pl = label("one partition on disk", 15, LOGC).move_to([-6.3, 2.3, 0], aligned_edge=LEFT)
        active = label("active segment: appends go here", 13, LOGC).next_to(segs[2], DOWN, buff=0.08)
        self.play(FadeIn(segs), *[FadeIn(l) for l in logs], FadeIn(pl), FadeIn(active), run_time=0.7)
        ptr = Pointer("consumer, slow").place(logs[0], 3, dy=0.75)
        self.play(FadeIn(ptr), run_time=0.3)
        l1 = label("a segment rolls at log.segment.bytes (1 GiB) or log.roll.hours (168 h); each file is named by the first offset it holds", 14, LOGC, width=12.6).move_to([-5.9, -0.45, 0], aligned_edge=LEFT)
        ...
        age = Counter("age of the oldest segment", 0, "h", MUTED, size=22).move_to([-6.3, -1.6, 0], aligned_edge=LEFT)
        ...
        self.play(ptr.to(logs[1], 0, dy=0.75), run_time=0.6)
        l2 = label("retention: whole segments older than log.retention.hours (168 h) or beyond log.retention.bytes are deleted; a consumer whose offset pointed into one is reset by auto.offset.reset", 14, FAIL, width=12.6).move_to(l1, aligned_edge=LEFT)
        ...
        clog = Log(-2.95, -1.35, capacity=14, name="topic account-balances, cleanup.policy=compact: the record is the key's latest state")
        ...
        headl = label("tail: latest record per key, offsets keep their gaps", 12, LOGC).move_to([clog.slot(0)[0], clog.y + 0.45, 0], aligned_edge=LEFT)
        self.play(FadeIn(headl), run_time=0.3)
        l3 = label("compaction: the cleaner rewrites old segments keeping only the newest record for each key; offsets stay, so a reader that starts at the beginning gets every key's latest value", 14, LOGC, width=12.6).move_to(l1, aligned_edge=LEFT)
        ...
        # --- a tombstone
        tomb = Square(SIDE, fill_opacity=0, stroke_color=KEY_B, stroke_width=2.5).move_to([clog.slot(len(clog.cells))[0], clog.y + 1.2, 0])
        cross = VGroup(Line(UL * 0.09, DR * 0.09, color=KEY_B, stroke_width=2), Line(UR * 0.09, DL * 0.09, color=KEY_B, stroke_width=2)).move_to(tomb)
        tl = label("tombstone: key bob, value null", 12, KEY_B).next_to(tomb, RIGHT, buff=0.1)
        ...
        dr = Counter("tombstone age", 0, "h", MUTED, size=22).move_to([3.4, -2.55, 0], aligned_edge=LEFT)
        ...
        l4 = label("delete: a record with the key and a null value is a tombstone; compaction removes the key's earlier records, and the tombstone itself after delete.retention.ms (24 h)", 14, KEY_B, width=12.6).move_to(l1, aligned_edge=LEFT)
