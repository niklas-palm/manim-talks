...
class Controller(TalkSlide):
    def construct(self):
        ...
        ctrls = VGroup(*[broker(n, 2.7, 0.8, SYNC if i == 0 else LOGC).move_to([x, 2.05, 0]) for i, (n, x) in enumerate(zip(("controller 1: active", "controller 2: standby", "controller 3: standby"), CX))])
        mlog = Log(-2.95, 0.55, capacity=14, name="__cluster_metadata: the cluster's state as a log, replicated by Raft to every controller")
        for c in (LOGC, LOGC, SYNC, LOGC, LOGC, LOGC):
            mlog.put(c)
        brks = VGroup(*[broker(f"broker {i + 1}", 2.7, 0.8).move_to([x, -1.35, 0]) for i, x in enumerate(CX)])
        self.play(FadeIn(ctrls), FadeIn(mlog), FadeIn(brks), run_time=0.7)
        l1 = label("3 or 5 controllers; one is active and appends, the others replay the same log and can take over; brokers fetch the log and keep their own copy", 14, LOGC, width=12.6).move_to([-5.9, -2.35, 0], aligned_edge=LEFT)
        ...
        # --- an event travels
        ev = label("broker 3 fenced: new leader for its partitions", 13, FAIL).move_to([-2.95, 1.35, 0], aligned_edge=LEFT)
        ...
        for k in (1, 2):
            ...
            self.play(FadeOut(c), Flash(ctrls[k][0], color=SYNC, flash_radius=1.2, num_lines=8), run_time=0.25)
        for k in (0, 1):
            c = cell.copy(); self.add(c)
            self.play(c.animate.move_to(brks[k][0].get_center()).scale(0.6), run_time=0.3)
            self.play(FadeOut(c), Flash(brks[k][0], color=LOGC, flash_radius=1.2, num_lines=8), run_time=0.25)
        l2 = label("a change is a record: the active controller appends it, the quorum commits it, and every broker fetches it in order; brokers never disagree about the cluster", 14, SYNC, width=12.6).move_to(l1, aligned_edge=LEFT)
        ...
        # --- KRaft only
        l3 = label("Kafka 4.0 (March 2025) runs only in this mode; ZooKeeper is gone. Kafka keeps its own state the way it keeps yours: as an append-only log", 14, TEXT, width=12.6).move_to(l1, aligned_edge=LEFT)
