...
class Partitions(TalkSlide):
    def construct(self):
        ...
        brokers = VGroup(*[broker(f"broker {i + 1}", 3.75, 1.55).move_to([x, BY, 0]) for i, x in enumerate(XS)])
        logs = [Log(x - 1.63, BY - 0.12, capacity=8, name=f"partition {i}") for i, x in enumerate(XS)]
        load = Gauge("broker 1\nwrite load", FAIL, 1.5).move_to([-6.35, BY, 0])
        ...
        rnd = random.Random(2)
        seq = [rnd.choice(KEYS) for _ in range(8)]
        for k, c in enumerate(seq):
            logs[0].append(self, c, source=prod, rt=0.2, extra=[load.set(0.15 + 0.12 * k)])
        l1 = label("one partition is one broker's disk and one ordering: the whole topic lands on one machine", 15, LOGC).move_to([-5.9, -0.25, 0], aligned_edge=LEFT)
        ...
        # --- split by key
        self.remove(*ghosts)
        hashl = label("partition = hash(key) mod 3", 15, TEXT).next_to(prod, RIGHT, buff=0.5)
        ...
        l2 = label("the same key always lands in the same partition, so records of one key stay in order; across partitions there is no order", 15, LOGC).move_to(l1, aligned_edge=LEFT)
        ...
        gl = label("consumer group: every partition is read by exactly one member", 14, READER).next_to(grp, UP, buff=0.06).align_to(grp, LEFT)
        ...
        newtag = label("c2", 12, READER).next_to(ptrs[2].tri, DOWN, buff=0.04)
        ...
        for step in range(3, 6):
            ...
            self.play(*[ptrs[i].to(logs[i], min(step + 1, len(logs[i].cells))) for i in range(3)], run_time=0.2)
        l3 = label("rebalance: the partition of a member that leaves is assigned to another. Classic protocol: the whole group pauses. 4.0 protocol (KIP-848): one partition moves, assigned by the broker", 14, READER, width=12.6).move_to([-5.9, -2.75, 0], aligned_edge=LEFT)
        ...
        olog = Log(-2.95, -2.55, capacity=14, name="__consumer_offsets: the group's committed positions, a compacted topic of 50 partitions")
        self.play(FadeIn(olog), run_time=0.5)
        for i, src in enumerate([cons[0], cons[1], cons[1], cons[0]]):
            d = Dot(color=READER, radius=0.07).move_to(src.get_bottom())
            self.add(d)
