...
def legend():
    return VGroup(*[VGroup(Square(0.16, fill_color=c, fill_opacity=0.9, stroke_width=0), label(f"key {NAMES[c]}", 12, MUTED)).arrange(RIGHT, buff=0.08)
...
class TheLog(TalkSlide):
    def construct(self):
        ...
        prod = producer().move_to([-5.4, 1.05, 0])
        brk = broker("broker", 8.4, 1.75).move_to([1.1, 1.0, 0])
        log = Log(-2.95, 0.9, capacity=14, name="topic payments, one partition")
        leg = legend().move_to([4.6, 2.55, 0])
        ...
        l1 = label("append-only: a record lands at the end, gets the next offset, and is never moved or changed", 15, LOGC).move_to([-5.9, -0.45, 0], aligned_edge=LEFT)
        ...
        c1 = consumer("consumer 1").move_to([-3.0, -1.6, 0])
        p1 = Pointer("consumer 1").place(log, 0)
        pos = Counter("consumer 1's position: the offset of the next record", 0, "", READER, size=22).move_to([0.4, -1.8, 0], aligned_edge=LEFT)
        ...
        l2 = label("pull: the consumer asks for records from its offset and advances the offset itself; the broker keeps no per-reader state", 15, READER).move_to(l1, aligned_edge=LEFT)
        ...
        c2 = consumer("consumer 2").move_to([-0.6, -1.6, 0])
        p2 = Pointer("consumer 2").place(log, 0, dy=0.98)
        self.play(FadeIn(c2), FadeIn(p2), run_time=0.4)
        for i in range(2):
            read_flash(self, log.cells[i], p2, c2, rt=0.25)
            self.play(p2.to(log, i + 1, dy=0.98), run_time=0.25)
        for c in (KEY_C, KEY_A):
            log.append(self, c, source=prod, rt=0.3)
        l3 = label("reading deletes nothing: every consumer keeps its own offset, and the producer keeps appending", 15, READER).move_to(l1, aligned_edge=LEFT)
        ...
        l4 = label("replay: move the offset back and read again; a new group starts at auto.offset.reset (latest by default, or earliest)", 15, READER).move_to(l1, aligned_edge=LEFT)
