...
SIDE, GAP = 0.42, 0.07          # a record cell and the gap between cells
PITCH = SIDE + GAP
...
class Log(VGroup):
    ...
    def __init__(self, x0: float, y: float, capacity: int = 12, base: int = 0, name: str = "", **kw):
        ...
        self.rail = Rectangle(width=capacity * PITCH + GAP, height=SIDE + 0.16, stroke_color=DIM, stroke_width=1.2, fill_opacity=0)
    ...
    def offset_label(self, i: int) -> Text:
        return label(str(self.base + i), 14, MUTED).move_to([self.slot(i)[0], self.y - SIDE / 2 - 0.22, 0])
