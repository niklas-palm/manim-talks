...
`bright`, a symlink to that talk's `scenes` and one to its `script.md`; a render adds its own `media/` and its two
pages. There is one copy of the drawing, so the two styles cannot drift apart. Render and present it like any other talk:
