...
sits with while the speaker talks: the fastest review of captions, spacing and colour there is.
...
for f in sorted(glob.glob(f"{root}/scenes/s*.py")):
    ...
    for scene in re.findall(r"^class (\w+)\(TalkSlide\)", open(f).read(), re.M):
        ...
        for k, sec in enumerate(json.load(open(idx))):
            ...
            subprocess.run(["ffmpeg", "-loglevel", "error", "-y", "-ss", f"{max(0, min(t - 0.15, dur - 0.15)):.3f}", "-i", video,
